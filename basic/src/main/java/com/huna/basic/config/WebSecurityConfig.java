...
import com.huna.basic.filter.JwtAuthenticationFilter;
...
public class WebSecurityConfig {
    ...
    // @Bean :
    // - Spring bean으로 등록하는 어노테이션, 인스턴스 생성X?, 클래스,메서드 다 사용가능한거임(메서드에서만 사용X)
    ...
    protected SecurityFilterChain configure(HttpSecurity security) throws Exception {
        ...
        // httpBasic이 메서드를원함 -> 클래스::disable이라는 매서드를 전달
        security
        // httpBasic : basic authentication 미사용
        .httpBasic(HttpBasicConfigurer::disable)
        // session : 
        // - 웹 애플리케이션에서 사용자에 대한 정보 및 상태를 유지하기 위한 기술
        // - 서버측에서 사용자 정보 및 상태를 저장하는 방법
        // - REST API 서버에서는 사용자 정보 및 상태를 클라이언트가 유지하기 때문에 session을 생성하지 않음 (밑의 코드)
        // cookie :
        // - 웹 애플리케이션에서 사용자에 대한 정보 및 상태를 유지하기 위한 기술
        // - 클라이언트측에서 사용자 정보 및 상태를 저장하는 방법
        // session과 cookie의 차이
        // - 저장위치 : cookie는 클라이언트(f12로 볼수 있ㅇ므), session은 서버(볼 수 없음)
        // - 보안 : session이 보안 수준이 높음
        // - 수명 : cookie는 지정한 기간동안 지속적으로 유지, session은 연결이 끊기면 파기됨
        // - 용도 : cookie에는 간단한 데이터(id)를 저장, session에는 민감한 데이터(개인정보)를 저장
        ...
        .authorizeHttpRequests(request -> request
            ...
            // "/student"과 "/studnet/**" 패턴 다름, 둘 모두 패턴 사용하고싶으면 "/student"도 꼭 적어줘야함
            .requestMatchers("/student", "/studnet/**").hasRole("STUDENT")
            // 인증된 사용자는 모두 접근을 허용

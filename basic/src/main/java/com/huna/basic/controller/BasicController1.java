...
import com.huna.basic.service.implement.BasicServiceImplement;
...
public class BasicController1 {
    ...
    // 원래 걸어야하지만 생성자일 때는 안 걸어도 됨
    // 의존성 역전 (DI) : 
    // - 해당 클래스에 필요한 의존성을 외부(클래스의 인스턴스를 생성하는 위치)에서 주입
    // - 3가지 방법 : 생성자를 통한 의존성 주입, setter 메서드를 통한 의존성 주입, 필드를 통한 의존성 주입
    ...
    // @Autowired
    // public BasicController1(BasicService service) {
    //     this.service = service;
    // }

    // 의존성 주입 시 클래스로 직접 참조변수를 만들지 않고 인터페이스로 간접적으로 만드는 이유 :
    // 고수준의 모듈에서 저수준의 모듈을 참조하지 않고 추상화를 통해 간접 참조함으로써 각 모듈간의 결합도를 낮춤
